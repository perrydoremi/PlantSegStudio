...
class RandomFlip3D(RandomFlip):
    ...
    def __init__(self,
        ...
        if flip_ratio_bev_horizontal is not None:
            assert isinstance(flip_ratio_bev_horizontal,
                              (int, float)) and 0 <= flip_ratio_bev_horizontal <= 1
        if flip_ratio_bev_vertical is not None:
            assert isinstance(flip_ratio_bev_vertical,
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if sampled_dict is not None:
            ...
            if self.sample_2d:
                ...
                gt_bboxes_2d = np.concatenate([gt_bboxes_2d,
...
class GlobalRotScaleTrans(BaseTransform):
    ...
    def _rot_bbox_points(self, input_dict: dict) -> None:
        ...
        if 'gt_bboxes_3d' in input_dict and \
            ...
            points, rot_mat_T = input_dict['gt_bboxes_3d'].rotate(noise_rotation,
                                                                  input_dict['points'])
            input_dict['points'] = points
        else:
...
@TRANSFORMS.register_module()
class InstanceInfoSoftGroup(BaseTransform):
    """Get instance information including mean points, instance numbers and
    classes.
    ...
    - pt_offset_labels (np.float32)
    - instance_num (int)
    - instance_pointnum (List[int])
    - instance_cls (List[int])
    ...
    def transform(self, input_dict: dict) -> dict:
        """Calculate instance information from point cloud data.

        Args:
            input_dict (dict): Result dict containing point cloud and label data.

        Returns:
            dict: Results after calculating instance info. Added keys include
            'pt_offset_labels', 'instance_num', 'instance_pointnum', and
            'instance_cls'.
        """

        # xyz = input_dict['points'][:, :3]
        if isinstance(input_dict, list):
            input_dict = input_dict[0]
        xyz = input_dict['points'].coord.numpy()
        # instance_label = input_dict['pts_semantic_mask']
        # semantic_label = input_dict['pts_semantic_mask']
        instance_label = input_dict.get('pts_instance_mask', None)
        semantic_label = input_dict.get('pts_semantic_mask', None)
        ...
        input_dict['pt_offset_labels'] = pt_offset_label
        input_dict['instance_num'] = instance_num
        input_dict['instance_pointnum'] = instance_pointnum
        input_dict['instance_cls'] = instance_cls
        return input_dict
...
class PointSample(BaseTransform):
    ...
    def _points_random_sampling(
            ...
            sample_range: Optional[float] = None,
            replace: bool = False,
            return_choices: bool = False) -> Union[Tuple[BasePoints, np.ndarray], BasePoints]:
...
class IndoorPatchPointSample(BaseTransform):
    ...
    def __init__(self,
        ...
        if sample_rate is not None:
            warnings.warn("'sample_rate' has been deprecated and will be removed in "
                          'the future. Please remove them from your code.')

    def _input_generation(self, coords: np.ndarray, patch_center: np.ndarray, coord_max: np.ndarray,
                          attributes: np.ndarray, attribute_dims: dict,
        ...
        if self.use_normalized_coord:
            ...
            attribute_dims.update(
                dict(normalized_coord=[
                    attributes.shape[1], attributes.shape[1] + 1, attributes.shape[1] + 2
                ]))
    ...
    def _patch_points_sampling(self, points: BasePoints,
        ...
        for _ in range(self.num_try):
            ...
            cur_choice = np.sum(
                (coords >= (cur_min - self.enlarge_size)) * (coords <=
                                                             (cur_max + self.enlarge_size)),
                axis=1) == 3
            ...
            if self.min_unique_num is None:
                ...
                vidx = np.ceil((cur_coords[mask, :] - cur_min) / (cur_max - cur_min) *
        ...
        points = self._input_generation(coords[choices], cur_center, coord_max, attributes[choices],
...
class VoxelBasedPointSampler(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        cur_sweep_points = self._sample_points(cur_sweep_points, self.cur_voxel_generator,
                                               points_numpy.shape[1])
        if self.prev_voxel_generator is not None:
            prev_sweeps_points = self._sample_points(prev_sweeps_points, self.prev_voxel_generator,
                                                     points_numpy.shape[1])
...
class AffineResize(BaseTransform):
    ...
    def __init__(self, img_scale: Tuple, down_ratio: int, bbox_clip_border: bool = True) -> None:
    ...
    def transform(self, results: dict) -> dict:
        ...
        if isinstance(self.down_ratio, tuple):
            trans_mat = [
                self._get_transform_matrix(center, size,
        ...
        if 'centers_2d' in results:
            ...
            if 'gt_bboxes_3d' in results:
                results['gt_bboxes_3d'].tensor = results['gt_bboxes_3d'].tensor[valid_index]
    ...
    def _get_ref_point(self, ref_point1: np.ndarray, ref_point2: np.ndarray) -> np.ndarray:
...
class RandomCrop3D(RandomCrop):
    ...
    def _crop_data(self,
        ...
        if isinstance(results['cam2img'], list):
            ...
            results['cam2img'][:offset_cam2img.shape[0], :offset_cam2img.shape[1]] = offset_cam2img
...
class PhotoMetricDistortion3D(PhotoMetricDistortion):
    ...
    def transform(self, results: dict) -> dict:
        ...
        (mode, brightness_flag, contrast_flag, saturation_flag, hue_flag, swap_flag, delta_value,
         alpha_value, saturation_value, hue_value, swap_value) = photometric_param
...
class MultiViewWrapper(BaseTransform):
    ...
    def __init__(
        ...
        collected_keys: list = [
            'scale', 'scale_factor', 'crop', 'img_crop_offset', 'ori_shape', 'pad_shape',
            'img_shape', 'pad_fixed_size', 'pad_size_divisor', 'flip', 'flip_direction', 'rotate'
        ],
        randomness_keys: list = [
            'scale', 'scale_factor', 'crop_size', 'img_crop_offset', 'flip', 'flip_direction',
            'photometric_param'
        ]
    ) -> None:
...
class PolarMix(BaseTransform):
    ...
    def polar_mix_transform(self, input_dict: dict, mix_results: dict) -> dict:
        ...
        if np.random.random() < self.rotate_paste_ratio:
            ...
            angle_list = [
                np.random.random() * np.pi * 2 / 3, (np.random.random() + 1) * np.pi * 2 / 3
            ]
...
class ElasticTransfrom(BaseTransform):
    ...
    def elastic(self, x, gran, mag):
        ...
        for blur in [blur0, blur1, blur2, blur0, blur1, blur2]:
            noise = [
                scipy.ndimage.filters.convolve(n, blur, mode='constant', cval=0) for n in noise
            ]
...
class AddSuperPointAnnotations(BaseTransform):
    ...
    def transform(self, input_dict):
        """Private function for preparation ground truth markup for training.
        ...
        """
        # create class mappinp because pts_instance_mask
        # contains instances from non-instaces classes
        ...
        if insts_mask.shape[1] != 0:
            ...
            sp_masks_inst = insts_mask.new_zeros((0, input_dict['sp_pts_mask'].max() + 1),
                                                 dtype=torch.bool)
        ...
        sem_mask = torch.nn.functional.one_hot(sem_mask, num_classes=self.num_classes + 1)
        ...
        if self.merge_non_stuff_cls:
            sp_masks_seg = torch.vstack((sp_masks_seg[:num_stuff_cls, :],
...
class SwapChairAndFloor(BaseTransform):
    """Swap two categories for ScanNet200 dataset.

    It is convenient for panoptic evaluation. After this swap first two
    categories are `stuff` and other 198 are `thing`.
    """
...
class PointInstClassMapping_(BaseTransform):
    ...
    def transform(self, input_dict):
        """Private function for deleting instances from non-instaces classes.
