...
class Pack3DDetInputs(BaseTransform):
    ...
    def __init__(
        self,
        keys: tuple,
        meta_keys: tuple = ('img_path', 'ori_shape', 'img_shape', 'lidar2img', 'depth2img',
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        for key in [
                ...
                'attr_labels', 'pts_instance_mask', 'pts_semantic_mask', 'centers_2d', 'depths',
                'gt_labels_3d'
        ]:
...
class Pack3DDetInputs_(Pack3DDetInputs):
    ...
    INSTANCEDATA_3D_KEYS = [
        'gt_bboxes_3d', 'gt_labels_3d', 'attr_labels', 'depths', 'centers_2d', 'gt_sp_masks'
    ]
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        for key in [
                'proposals', 'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels', 'gt_bboxes_labels',
                'attr_labels', 'pts_instance_mask', 'pts_semantic_mask', 'sp_pts_mask',
                'gt_sp_masks', 'elastic_coords', 'centers_2d', 'depths', 'gt_labels_3d'
        ]:
            if key not in results:
                continue
            if isinstance(results[key], list):
...
@TRANSFORMS.register_module()
class Pack3DSoftGroupInputs_(Pack3DDetInputs):
    """Pack inputs for softgroup, elastic_coords added in oneformer3d.
    ...
    # Additional instance info keys from SoftGroup
    INSTANCE_INFO_KEYS = ['pt_offset_labels', 'instance_num', 'instance_pointnum', 'instance_cls']

    def pack_single_results(self, results: dict) -> dict:
        """Pack the single input data including instance information.
        ...
        Returns:
            dict: A dict contains:
            - 'inputs': The forward data of models
            - 'data_samples': The annotation info of the sample
        """
        # Format 3D data
        if 'points' in results:
            if isinstance(results['points'], BasePoints):
        ...
        # Convert instance info and other data to tensor
        for key in [
                'proposals', 'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels', 'gt_bboxes_labels',
                'attr_labels', 'pts_instance_mask', 'pts_semantic_mask', 'elastic_coords',
                'gt_labels_3d', 'pt_offset_labels', 'instance_num', 'instance_pointnum',
                'instance_cls'
        ]:
            if key not in results:
                continue
            if key in self.INSTANCE_INFO_KEYS:
                continue
            if isinstance(results[key], list):
                results[key] = [to_tensor(res) for res in results[key]]
            else:
        ...
        for key in self.keys:
            if key in results:
                if key in self.INPUTS_KEYS:
                    ...
                    gt_instances_3d[self._remove_prefix(key)] = results[key]
                elif key in self.INSTANCE_INFO_KEYS:
                    gt_instances_softgroup[key] = results[key]
                elif key in self.INSTANCEDATA_2D_KEYS:
                    if key == 'gt_bboxes_labels':
                        gt_instances['labels'] = results[key]
                    else:
                    ...
                    raise NotImplementedError(f'Please modify '
                                              f'`Pack3DSoftGroupInputs` '
                                              f'to put {key} to '
                                              f'corresponding field')

        data_sample.gt_instances_3d = gt_instances_3d
        data_sample.gt_instances_softgroup = gt_instances_softgroup
        data_sample.gt_instances = gt_instances
        data_sample.gt_pts_seg = gt_pts_seg
        if 'eval_ann_info' in results:
            data_sample.eval_ann_info = results['eval_ann_info']
