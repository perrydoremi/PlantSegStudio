...
from mmdet3d.models.layers.softgroup_modules import MLP, ResidualBlock, UBlock
from mmdet3d.structures import PointData
from ..ops.softgroup_ops import (ball_query, bfs_cluster, get_mask_iou_on_cluster,
                                 get_mask_iou_on_pred, get_mask_label, global_avg_pool, sec_max,
                                 sec_min, voxelization, voxelization_idx)
from ..utils.softgroup_utils import cuda_cast, force_fp32, rle_decode, rle_encode


@MODELS.register_module()
class SoftGroup(Base3DDetector):
    r"""SoftGroup model refactored from thangvubk/SoftGroup.
    Please refer to the `paper <https://arxiv.org/abs/2209.08263>`_

    Args:
        in_channels (int): Number of input channels.
        num_channels (int): Number of output channels.
        num_blocks (int): Number of blocks in the backbone.
        semantic_only (bool): Whether to perform semantic segmentation only.
        semantic_classes (int): Number of semantic classes.
        instance_classes (int): Number of instance classes.
        semantic_weight (list or None): Class weights for semantic loss.
        sem2ins_classes (list): List of classes for semantic-to-instance mapping.
        ignore_label (int): Label to ignore during loss computation.
        with_color (bool): Whether to include color in input features.
        grouping_cfg (dict): Configuration for the grouping module.
        instance_voxel_cfg (dict): Configuration for instance voxelization.
        train_cfg (dict, optional): Training configuration.
        test_cfg (dict, optional): Testing configuration.
        data_preprocessor (dict or ConfigDict, optional): Data preprocessor configuration.
        init_cfg (dict or ConfigDict, optional): Initialization configuration.
    """
    ...
    def __init__(self,
        ...
        assert self.backbone.type in \
            ['MinkUNetBackboneV2', 'SPVCNNBackbone', 'SoftGroupUNet']
    ...
    def _init_layers(self):
        ...
        block_channels = [self.channels * (i + 1) for i in range(self.num_blocks)]
        if self.backbone.type == 'SoftGroupUNet':
            self.unet = UBlock(block_channels, norm_fn, 2, block, indice_key_id=1)
        else:
            self.unet = MODELS.build(self.backbone)
        self.output_layer = spconv.SparseSequential(norm_fn(self.channels), torch.nn.ReLU())

        # Point-wise prediction
        self.semantic_linear = MLP(
            self.channels, self.semantic_classes, norm_fn=norm_fn, num_layers=2)

        self.offset_linear = MLP(self.channels, 3, norm_fn=norm_fn, num_layers=2)

        # Top-down refinement
        if not self.semantic_only:
            self.tiny_unet = UBlock([self.channels, 2 * self.channels],
                                    norm_fn,
                                    2,
                                    block,
                                    indice_key_id=11)
            ...
            self.tiny_unet_outputlayer = spconv.SparseSequential(
                norm_fn(self.channels), torch.nn.ReLU())

            self.cls_linear = torch.nn.Linear(self.channels, self.instance_classes + 1)

            self.mask_linear = MLP(
                self.channels, self.instance_classes + 1, norm_fn=None, num_layers=2)

            self.iou_score_linear = torch.nn.Linear(self.channels, self.instance_classes + 1)
    ...
    def extract_feat(self, input, input_map, lvl_fusion=False):
        """Extract features from input tensor and compute semantic scores and
        offsets. forward backbone of the original softgroup model.

        Args:
            input (SparseTensor): Input sparse tensor
            input_map: Mapping tensor for feature sampling
            x4_split (bool): Not used in this version
            lvl_fusion (bool): Whether to use level fusion

        Returns:
            tuple: (semantic_scores, pt_offsets, output_feats)
        """
        if self.backbone.type in ['MinkUNetBackboneV2', 'SPVCNNBackbone']:
            output = self.unet(input['voxels'], input['coors'])
            output_feats = output.F
        elif self.backbone.type == 'SoftGroupUNet':
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        Args:
            batch_inputs_dict (dict): Input dict.
            batch_data_samples (List[Det3DDataSample]): List of data samples.
        ...
        batch_idxs = batch_inputs_dict['batch_idxs']
        voxel_feats = voxelization(batch_inputs_dict['feats'], batch_inputs_dict['p2v_map'])
        if self.backbone.type in ['MinkUNetBackboneV2', 'SPVCNNBackbone']:
            input = dict(
                voxels=voxel_feats, coors=batch_inputs_dict['voxel_coords'][:, [1, 2, 3, 0]])
        elif self.backbone.type == 'SoftGroupUNet':
            input = spconv.SparseConvTensor(voxel_feats, batch_inputs_dict['voxel_coords'].int(),
                                            batch_inputs_dict['spatial_shape'],
                                            batch_inputs_dict['batch_size'])
        # Extract features
        semantic_scores, pt_offsets, output_feats = self.extract_feat(input,
                                                                      batch_inputs_dict['v2p_map'])

        # Semantic and offset losses
        point_wise_loss = self.point_wise_loss(semantic_scores, pt_offsets,
                                               batch_inputs_dict['semantic_labels'],
                                               batch_inputs_dict['instance_labels'],
                                               batch_inputs_dict['pt_offset_labels'])
        losses.update(point_wise_loss)

        # Instance losses
        if not self.semantic_only:
            proposals_idx, proposals_offset = self.forward_grouping(
                semantic_scores, pt_offsets, batch_idxs, batch_inputs_dict['coords_float'],
                self.grouping_cfg)
            ...
            inst_feats, inst_map = self.clusters_voxelization(
                ...
                proposals_offset,
                output_feats,
                batch_inputs_dict['coords_float'],
                rand_quantize=True,
            ...
            instance_loss = self.instance_loss(cls_scores, mask_scores, iou_scores, proposals_idx,
                                               proposals_offset,
                                               batch_inputs_dict['instance_labels'],
                                               batch_inputs_dict['instance_pointnum'],
                                               batch_inputs_dict['instance_cls'],
                                               instance_batch_idxs)
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        """
        batch_idxs = batch_inputs_dict['batch_idxs']
        coords_float = batch_inputs_dict['coords_float']
        # Decorated fake color features
        color_feats = batch_inputs_dict[
            'feats'][:, [3, 4, 5]] if self.with_color else batch_inputs_dict['feats']
        voxel_feats = voxelization(batch_inputs_dict['feats'], batch_inputs_dict['p2v_map'])
        if self.backbone.type in ['MinkUNetBackboneV2', 'SPVCNNBackbone']:
            input = dict(
                voxels=voxel_feats, coors=batch_inputs_dict['voxel_coords'][:, [1, 2, 3, 0]])
        ...
        # Extract features
        semantic_scores, pt_offsets, output_feats = self.extract_feat(
            input, batch_inputs_dict['v2p_map'], lvl_fusion=lvl_fusion)

        semantic_preds = semantic_scores.max(1)[1]

        results = PointData(scan_id=batch_inputs_dict['scan_ids'][0])
        # dict(scan_id=batch_inputs_dict['scan_ids'][0])
        # Get instance predictions
        if 'semantic' in self.test_cfg.eval_tasks:
            results.semantic_labels = batch_inputs_dict['semantic_labels'].cpu().numpy()
            results.instance_labels = batch_inputs_dict['instance_labels'].cpu().numpy()
        if 'semantic' in self.test_cfg.eval_tasks:
            point_wise_results = self.get_point_wise_results(coords_float, color_feats,
                                                             ...
                                                             batch_inputs_dict['v2p_map'],
                                                             lvl_fusion)
            results.point_wise_results = point_wise_results

        if not self.semantic_only:
            if 'instance' in self.test_cfg.eval_tasks:
                if lvl_fusion:
                    batch_idxs = input.indices[:, 0].int()
                    coords_float = voxelization(coords_float, batch_inputs_dict['p2v_map'])
                proposals_idx, proposals_offset = self.forward_grouping(
                    semantic_scores,
                    pt_offsets,
                    batch_idxs,
                ...
                # print(f"prediciting {batch_inputs_dict['scan_ids'][0]}")
                # print(f"proposals_idx: {proposals_idx}, proposals_offset: {proposals_offset}")
                ...
                pred_instances = self.get_instances(
                    batch_inputs_dict['scan_ids'][0],
                    proposals_idx,
                    semantic_scores,
                    cls_scores,
                    iou_scores,
                    mask_scores,
                    v2p_map=batch_inputs_dict['v2p_map'],
                    lvl_fusion=lvl_fusion)
            if 'instance' in self.test_cfg.eval_tasks:
                gt_instances = self.get_gt_instances(batch_inputs_dict['semantic_labels'],
                                                     batch_inputs_dict['instance_labels'])
            ...
            #     results.panoptic_preds=panoptic_preds

            # Update data samples
            batch_data_samples[0].pred_pts_seg = results
    ...
    def forward_grouping(self,
        ...
        for class_id in range(self.semantic_classes):
            try:
                ...
                # Add safety checks before ball query
                if coords_.size(0) == 0:
                    continue
                ...
                proposals_idx, proposals_offset = bfs_cluster(class_numpoint_mean,
                                                              neighbor_inds.cpu(), start_len.cpu(),
                                                              npoint_thr, class_id)
                ...
                print(f'Skipping current sample in class {class_id} due to error: {str(e)} ')
                continue
