...
class InstanceCriterion:
    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        if indices is None:
            ...
            for i in range(len(insts)):
                pred_instances = InstanceData_(
                    scores=cls_preds[i], masks=pred_masks[i])
        ...
        for cls_pred, inst, (idx_q, idx_gt) in zip(cls_preds, insts, indices):
            ...
            cls_target = cls_pred.new_full((len(cls_pred), ),
                                           n_classes,
                                           dtype=torch.long)
            cls_target[idx_q] = inst.labels_3d[idx_gt]
            cls_losses.append(
                F.cross_entropy(cls_pred, cls_target,
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores,
            ...
            mask_bce_losses.append(
                F.binary_cross_entropy_with_logits(pred_mask,
                                                   tgt_mask.float()))
        ...
        if len(mask_bce_losses):
            mask_bce_loss = torch.stack(mask_bce_losses).sum() / len(
                pred_masks)
            mask_dice_loss = torch.stack(mask_dice_losses).sum() / len(
                pred_masks)
...
class QueryClassificationCost:
    ...
    def __call__(self, pred_instances, gt_instances, **kwargs):
        ...
        Args:
            ...
            gt_instances (:obj:`InstanceData_`):
                Ground truth which must contain `labels` of shape (n_gts,).
...
class MaskBCECost:
    ...
    def __call__(self, pred_instances, gt_instances, **kwargs):
        ...
        Args:
            ...
            gt_instances (:obj:`InstanceData_`):
                Ground truth which must contain
        ...
        Returns:
            Tensor: Cost of shape (n_queries, n_gts).
        """
        cost = batch_sigmoid_bce_loss(pred_instances.masks,
                                      gt_instances.masks.float())
        return cost * self.weight
...
class MaskDiceCost:
    ...
    def __call__(self, pred_instances, gt_instances, **kwargs):
        ...
        Returns:
            Tensor: Cost of shape (n_queries, n_gts).
        """
        cost = batch_dice_loss(pred_instances.masks,
                               gt_instances.masks.float())
        return cost * self.weight
...
class SparseMatcher:
    ...
    def __call__(self, pred_instances, gt_instances, **kwargs):
        ...
        cost_value = torch.stack(cost_values).sum(dim=0)
        cost_value = torch.where(gt_instances.query_masks.T, cost_value,
                                 self.inf)
...
class OneDataCriterion:
    ...
    def __init__(self, matcher, loss_weight, non_object_weight,
        ...
        self.class_weight_1dataset = [1] * num_classes_1dataset + [
            non_object_weight
        ]
        self.class_weight_2dataset = [1] * num_classes_2dataset + [
            non_object_weight
        ]
        self.loss_weight = loss_weight
        self.fix_dice_loss_weight = fix_dice_loss_weight
