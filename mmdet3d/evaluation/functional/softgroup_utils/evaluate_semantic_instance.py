...
from ....models.utils.softgroup_utils import rle_decode
...
def evaluate_matches(matches, class_labels, options):
    ...
    pr_rc = np.zeros((2, len(class_labels), len(overlaps)), float)
...
def compute_averages(aps, pr_rc, options, class_labels):
    ...
    """
    d_inf = 0
    o90 = np.where(np.isclose(options['overlaps'], 0.9))
    ...
    avg_dict['all_ap_90%'] = np.nanmean(aps[d_inf, :, o90])
    ...
    for (li, label_name) in enumerate(class_labels):
        ...
        avg_dict['classes'][label_name]['ap90%'] = np.average(aps[d_inf, li, o90])
        avg_dict['classes'][label_name]['ap50%'] = np.average(aps[d_inf, li, o50])
        avg_dict['classes'][label_name]['ap25%'] = np.average(aps[d_inf, li, o25])
        avg_dict['classes'][label_name]['prec50%'] = np.average(pr_rc[0, li, o50])
        avg_dict['classes'][label_name]['rec50%'] = np.average(pr_rc[1, li, o50])
    return avg_dict


def assign_instances_for_scan(preds, gts, options, valid_class_ids, class_labels, id_to_label):
    """Assign gt and predicted instances for a single scene.

    Args:
        preds (dict): Predicted masks, labels and scores.
        gts (np.array): Ground truth instance masks.
        options (dict): ScanNet evaluator options. See get_options.
    ...
    """
    # get gt instances
    gt_instances = util_3d.get_instances(gts, valid_class_ids, class_labels, id_to_label)
    # decorated dict to skip the first instance class
    id_to_label_inst = {k: v for k, v in id_to_label.items() if k != min(id_to_label.keys())}
    # associate
    ...
    bool_void = np.logical_not(np.in1d(gts // 1000, valid_class_ids))
    # go through all prediction masks
    for pred in preds:
        # if self.use_label:
        label_id = pred['label_id']
        if label_id not in id_to_label_inst:
            continue
        label_name = id_to_label_inst[label_id]
...
def scannet_eval_softgroup(preds, gts, options, valid_class_ids, class_labels, id_to_label):
    ...
    for i, (pred, gt) in enumerate(zip(preds, gts)):
        ...
        gt2pred, pred2gt = assign_instances_for_scan(pred, gt, options, valid_class_ids,
