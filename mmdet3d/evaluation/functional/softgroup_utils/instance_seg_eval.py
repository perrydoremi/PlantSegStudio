...
from mmdet3d.evaluation.functional.softgroup_utils.evaluate_semantic_instance import \
    scannet_eval_softgroup  # noqa: E501
...
def instance_seg_eval_softgroup(preds,
                                gts,
                                valid_class_ids,
                                class_labels,
                                options=None,
                                logger=None):
    """Instance Segmentation Evaluation for softgroup.

    Evaluate the result of the instance segmentation.

    Args:
        preds (list[dict]): Per scene predicted instance masks.
        gts (list[np.ndarray]): Per scene gt instance masks.
        valid_class_ids (tuple[int]): Ids of valid categories.
    ...
    metrics = scannet_eval_softgroup(
        preds=preds,
        gts=gts,
        options=options,
        valid_class_ids=valid_class_ids,
    ...
    aps = metrics['all_ap_25%'], metrics['all_ap_50%'], metrics['all_ap'], metrics[
        'all_prec_50%'], metrics['all_rec_50%']
