...
from mmdet3d.evaluation.functional.oneformer3d_utils.evaluate_semantic_instance import \
    scannet_eval  # noqa: E501
...
def instance_seg_eval(gt_semantic_masks,
    ...
    for label, data in metrics['classes'].items():
        aps = [
            data['ap25%'], data['ap50%'], data['ap'], data['prec50%'],
            data['rec50%']
        ]
        rows.append([label] + [f'{ap:.4f}' for ap in aps])
    aps = metrics['all_ap_25%'], metrics['all_ap_50%'], metrics[
        'all_ap'], metrics['all_prec_50%'], metrics['all_rec_50%']
