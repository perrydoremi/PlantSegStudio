...
def update_plant_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    print(f'Reading from input file: {pkl_path}.')
    data_list = mmengine.load(pkl_path)
    METAINFO = tuple(data_list[0]['meta_instance'])
    print('Start updating:')
    converted_list = []
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list)):
        temp_data_info = get_empty_standard_data_info()
        temp_data_info['sample_idx'] = ori_info_dict['point_cloud'][
            'lidar_idx']
        temp_data_info['lidar_points']['num_pts_feats'] = ori_info_dict[
            'point_cloud']['num_features']
        temp_data_info['lidar_points']['lidar_path'] = Path(
        ...
        if anns is not None:
            if anns['gt_num'] == 0:
                ...
                for instance_id in range(num_instances):
                    ...
                    if anns['class'][instance_id] < len(METAINFO):
                        empty_instance['bbox_label_3d'] = anns['class'][
                            instance_id]
                    else:
                        ignore_class_name.add(
                            METAINFO[anns['class'][instance_id]])
                        empty_instance['bbox_label_3d'] = -1
    ...
    if ignore_class_name:
        for ignore_class in ignore_class_name:
            metainfo['categories'][ignore_class] = -1
    metainfo['dataset'] = data_list[0]['dataset']
    metainfo['info_version'] = '1.1'
...
def update_plant_pkl_infos(out_dir, pkl_path):
    update_plant_infos(pkl_path=pkl_path, out_dir=out_dir)
