...
def parse_args():
    ...
    parser.add_argument(
        '--samples', default=1000, type=int, help='samples to benchmark')
    parser.add_argument(
...
def main():
    ...
    for i, data in enumerate(dataloader):
        print(data['data_samples'][0].lidar_path)
        ...
        if (i + 1) == samples:
            pure_inf_time += elapsed
            fps = (i + 1 - num_warmup) / pure_inf_time
            print(f'Overall fps: {fps:.2f} sample / s')
            break
