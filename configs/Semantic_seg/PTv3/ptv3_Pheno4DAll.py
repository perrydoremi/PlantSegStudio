_base_ = [
    '../../_base_/models/ptv3.py',
    '../../_base_/schedules/seg-adamw-ptv3-252e.py',
    '../../_base_/default_runtime.py'
]
# Dataset settings
plant_type = 'Pheno4DAll'
use_color = False
point_cloud_range = [0, 0, 0, 414, 382, 470]
voxel_size = [1, 1, 1]
load_dim = 6 if use_color else 3
use_dim = list(range(6)) if use_color else list(range(3))
num_classes = 3

train_area = list(range(1, 36)) + list(range(50, 105))
test_area = list(range(36, 50)) + list(range(105, 127))
backend_args = None
# Model settings
# enable_flash=True with parity bs=16 at the native voxel size (flash attention
# is O(N) in memory); best variant used drop_path=0.1 at patch size 128.
bs = 16
max_epoch = 250
val_interval = 4

# Point-based dataset config
class_names = ('ground', 'stem', 'leaf')
